 ...
 * @param {string} [o.sellingPrice] - Precio recomendado de venta (RSP)
 * @param {Array<{subtype?:string,description?:string,amount?:string,currency?:string,label?:string}>} [o.excludedTaxes]
...
export function buildHotelbedsVoucherHtml(o) {
  ...
  const excludedTaxes =
    Array.isArray(o.excludedTaxes) && o.excludedTaxes.length
      ? `<div class="voucher-block"><h3>Impuestos y tasas no incluidos</h3><ul style="margin:4px 0;padding-left:20px;">${o.excludedTaxes
          .map((t) => {
            const line =
              t.label ||
              `${t.subtype || t.description || 'Tasa'}: ${t.amount || '—'} ${t.currency || 'EUR'}`;
            return `<li>${escapeHtml(line)}</li>`;
          })
          .join('')}</ul></div>`
      : '';

  const rsp = o.sellingPrice
    ? `<p style="margin: 4px 0;"><strong>Precio recomendado de venta (RSP):</strong> ${escapeHtml(o.sellingPrice)}</p>`
    : '';
...
export function buildHotelbedsVoucherPlainText(o) {
  ...
  if (Array.isArray(o.excludedTaxes) && o.excludedTaxes.length) {
    lines.push('', 'IMPUESTOS Y TASAS NO INCLUIDOS');
    o.excludedTaxes.forEach((t) => {
      lines.push(
        t.label || `${t.subtype || t.description || 'Tasa'}: ${t.amount || '—'} ${t.currency || 'EUR'}`
      );
    });
  }
  if (o.sellingPrice) {
    lines.push('', `Precio recomendado de venta (RSP): ${o.sellingPrice}`);
  }
...
export function voucherDataFromStripeMetadata(metadata) {
  ...
  let excludedTaxes = [];
  if (metadata.hb_excluded_taxes) {
    try {
      const parsed = JSON.parse(metadata.hb_excluded_taxes);
      if (Array.isArray(parsed)) excludedTaxes = parsed;
    } catch {
      excludedTaxes = [];
    }
  }
  ...
  return {
    ...
    sellingPrice: metadata.hb_selling_price || '',
    excludedTaxes,
...
export function voucherToStripeMetadata(v) {
  ...
  if (v.sellingPrice || v.selling_price) {
    out.hb_selling_price = slice(v.sellingPrice || v.selling_price, 120);
  }
  if (v.excludedTaxes || v.excluded_taxes) {
    const taxes = v.excludedTaxes || v.excluded_taxes;
    if (Array.isArray(taxes) && taxes.length) {
      out.hb_excluded_taxes = slice(JSON.stringify(taxes), META_MAX);
    }
  }
