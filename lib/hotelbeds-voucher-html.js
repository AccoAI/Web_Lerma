...
export function buildHotelbedsVoucherHtml(o) {
  ...
  const vatLine = supplierVat ? `VAT: ${escapeHtml(supplierVat)}. ` : '';
  const legal = `Payable through ${escapeHtml(supplierName)}, acting as agent for the service operating company, details of which can be provided upon request. ${vatLine}Reference: ${ref}.`;
...
<div class="hotelbeds-voucher" style="font-family: Georgia, 'Times New Roman', serif; max-width: 640px; margin: 0 auto; color: #1a1a1a;">
  <div style="border: 2px solid #2c5530; padding: 24px; background: #fafafa;">
    ...
    <div class="voucher-legal" style="margin-top: 24px; padding: 14px; background: #eef2ee; font-size: 13px; line-height: 1.45;">
      ...
      <p style="margin: 8px 0 0;">${legal}</p>
    </div>
...
export function buildHotelbedsVoucherPlainText(o) {
  ...
  const vatPart = supplierVat ? `VAT: ${supplierVat}. ` : '';
  lines.push(
    '',
    'INFORMACIÓN DE PAGO',
    `Payable through ${supplierName}, acting as agent for the service operating company, details of which can be provided upon request. ${vatPart}Reference: ${o.bookingReference}.`,
    '',
