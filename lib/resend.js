...
export async function sendEmail({ to, subject, html, from, text, attachments }) {
  ...
  if (domain && UNVERIFIABLE_DOMAINS.some((d) => domain === d || domain.endsWith('.' + d))) {
    console.error('Resend: remitente en dominio no verificable', domain);
    return {
      error:
  ...
  if (!res.ok) {
    console.error('Resend error', res.status, data, 'from=', fromAddress, 'to=', payload.to);
